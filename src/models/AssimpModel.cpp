...
AssimpModel::AssimpModel(const char* onbjectPath, const char* texturePath) : Model(0, nullptr)
{
  load(onbjectPath, texturePath);
}

AssimpModel::AssimpModel(const char* objectPath, Material* material) : Model(0, material)
{
  load(objectPath, "");
}
...
void AssimpModel::load(std::string objectPath, std::string texturePath) {
  ...
  const aiScene* scene = importer.ReadFile(objectPath, importOptions);

  if (scene) { //pokud bylo nacteni uspesne
    printf("scene->mNumMeshes = %d\n", scene->mNumMeshes);
    printf("scene->mNumMaterials = %d\n", scene->mNumMaterials);
    //Material
    if (scene->HasMaterials() && this->material == nullptr) {
      const aiMaterial* mat = scene->mMaterials[1];
      ...
      this->material = MaterialLib::getMaterial(texturePath, ambient, diffuse, specular, shininess);
    }
    else if (this->material == nullptr) {
      if (texturePath != "")
        this->material = MaterialLib::Base(texturePath);
      else
        this->material = MaterialLib::Base();
    }
